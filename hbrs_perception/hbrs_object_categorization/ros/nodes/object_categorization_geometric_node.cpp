...
std::string nodeName("hbrs_object_categorization_geometric");
...
void pointCloudVectorTrackingCallback(const hbrs_msgs::ObjectListPtr& pointCloudVectorMsg)
{
    pcl::PointCloud<pcl::PointXYZ> pointCloud;
	...
	if (pointCloudVectorMsg->objects.size() > 0)
	{
		...
		/**********************************/
		ROS_DEBUG("pointCloudVectorTrackingCallback...........................................................tracked and started ... took %lf",float(clock() - begin_time) / CLOCKS_PER_SEC);
	}
}

void pointCloudVectorGasCallback(const hbrs_msgs::ObjectListPtr& pointCloudVectorMsg)
{
	pcl::PointCloud<pcl::PointXYZ> pointCloud;
	clock_t begin_time = clock();
	...
	if (pointCloudVectorMsg->objects.size() > 0)
	{
		...
		for (unsigned int i = 0; i < pointCloudVectorMsg->objects.size(); i++)
		{
			...
			ROS_DEBUG("pointCloudVectorCallback...OBJECT NUMBER  %d/%d with %d points",(int)i,(int)pointCloudVectorMsg->objects.size(), (int)pointCloud.points.size() );
...
void newCategoryLearner(const hbrs_msgs::ObjectListPtr& pointCloudVectorMsg)
{
    pcl::PointCloud<pcl::PointXYZ> pointCloud;
...
int main(int argc, char **argv)
{
    ...
    pub_object_list = nodeHandle.advertise<hbrs_msgs::ObjectList> ("categorized_objects", 10);
	...
	//pointCloudCategorizationGeometricPublisher = nodeHandle.advertise<sensor_msgs::PointCloud2> (nodeName + "/object_categorization_pointcloud", 1);
	
	//load launch file parameter
	if (nodeHandle.getParam("home_path", homePath) == false)
		ROS_INFO(" ... could not read parameter /homePath from launch file");
	else
		ROS_INFO(" ... home path set to %s ", homePath.c_str());

	//init logger and categorizer with home path
	CLogger *logger = &CLogger::getInstance(homePath);
	...
	objectCategorizationGeometric->setHomePath(homePath + std::string("data/"));
