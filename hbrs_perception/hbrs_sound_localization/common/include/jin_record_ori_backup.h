...
public:
  ...
  void  data_recording()
  {
	 ...
	 for (int i=0,j=0;i<44100*4,j<44100;j++,i=i+4)
			{
				source[j][0]= data.buffer [i];
				source[j][1]= data.buffer [i+1];
				source[j][2]= data.buffer [i+2];
				source[j][3]= data.buffer [i+3];
...
///////////////////////////////////////////////////////////
    ...
    //record_obj->data_recording();
      ...
      this->final_position=new double[3];
			...
			this->source = (double **) malloc(this->Rows *sizeof(double));
			for(int iRow=0;iRow < Rows;iRow++)
				this->source[iRow] = (double *) malloc(this->Cols*sizeof(double));
	...
	void Localize_algorithm(double ** source){
	//////////////////////////////////
	
			////////////////downsampling or get one sec data//////////////////////////
		//	source=record_obj->get_source_data();
			/////////////////////////////////////////////////////////////////////
			int		row,col,col2;
			int		p=0;
...
void src(double* bstart,double* bend,double** mic_loc,double magiconst,double** yv_spline,double efsintp,double *final_position)
{
	...
	const int maxFEs=55000;
	fe_cal *array_result[randpts];
	...
	for(int i=0;i < randpts;i++)
	{
		...
		array_result[i]=new fe_cal( yv_spline,mic_loc,magiconst,efsintp,N,bstart,bend,i*7);
    ...
    size_t ii, kkgsl = npoints, NN = randpts;
      ...
      //for(int i=0;i<npoints;i++)
       ...
       //ind_vec=large_index;
		...
		double minVoxel=0.0001;//100*pow10(-6.0);
			...
			{		//cout<<"in small loop "<< nonewpts[kk]<<endl;
					for (int i=0;i< nonewpts[kk];i++)
					{
							...
							while (a4 < tmp_sum && countfe <=maxFEs)
							{
								...
								fe_cal *a4_good;
								a4_good	=new fe_cal( yv_spline,mic_loc,magiconst,efsintp,N,newbstart,newbend,countfe);
								a4=a4_good->get_yval ();
