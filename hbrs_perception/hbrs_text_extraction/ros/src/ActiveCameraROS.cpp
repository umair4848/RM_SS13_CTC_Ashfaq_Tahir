...
namespace atie {
    ...
    : nh()
  {
    ...
    iSubscriber = iTransport->subscribe(imageTopic_, 1, 
					&ActiveCameraROS::imageCallback, this);    
  }   
  ...
  int ActiveCameraROS::setParameters(int n_, char** names_, int* values_) {
    ...
    parameterSetSyncTime= boost::posix_time::microsec_clock::universal_time() +
      boost::posix_time::time_duration(0, 0, 0, maxWait);
    frameMutex.unlock();
    return 0;
  }
  ...
  void ActiveCameraROS::imageCallback(const sensor_msgs::ImageConstPtr& msg_ptr) {
    ...
    if (transcientStateP) {
	  ...
	  parameterSetSyncTime) {
	cv::Mat bridgeMat = bridge.imgMsgToCv(msg_ptr, "bgr8");	
