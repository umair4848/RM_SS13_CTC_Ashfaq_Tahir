...
IplImage* chenNiblackBinarization(IplImage* image, IplImage* result, 
				  float Td=configurableParameters.chenNiblackTd,
				  float k=configurableParameters.chenNiblackK,
				  int R= configurableParameters.chenNiblackMaxRadius,
				  int searchMethod=CHEN_NIBLACK_BINARY_SEARCH);
int estimateRSequential(IplImage* image, IplImage* sums, 
			IplImage* ssums, CvPoint* center, bool* homogeneous, 
			double Td, double* mean, double* std, 
			int R= configurableParameters.chenNiblackMaxRadius);

int estimateRBSearch(IplImage* image, IplImage* sums, 
		     IplImage* ssums, CvPoint* center, bool* homogeneous,
		     double Td, double* mean, double* std, 
		     int R= CHEN_NIBLACK_MAX_RADIUS);

void  chenNiblackBinarizePixel(IplImage* image, IplImage* sums, IplImage* ssums, 
			      IplImage* result, CvPoint* center, 
			       double Td, float k, int R=configurableParameters.chenNiblackMaxRadius, 
			       int searchMethod=CHEN_NIBLACK_BINARY_SEARCH);
