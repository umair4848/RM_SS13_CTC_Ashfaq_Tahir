...
class FeatureExtractor {
 protected:
  ...
  double* normalizationParameters;
  real* features;
 ...
 public:
  ...
  int fillSVMInputVector(struct svm_node* featureVector, 
			  double* (extractor) (T* ccd, int* n),
			  T* ccd, int fCount=0);
  double inline normalizeSVMFeature(double value, double featureMin, double featureMax, 
...
class CCBasicFeatureExtractor : public FeatureExtractor<ccRelatedData> {
 ...
 public:
  ...
  CCBasicFeatureExtractor(char* normalizationParametersPath= NULL,
		     int n=0, bool forTraining=false);
...
class CCFeatureExtractorCM : public CCBasicFeatureExtractor {
 ...
 public:
  CCFeatureExtractorCM(char* normalizationParametersPath= NULL,
		       int n=0, int nZernike=-1, int nPseudoZernike=-1, 
...
class CCFeatureExtractor : public CCBasicFeatureExtractor {
 public:
  CCFeatureExtractor(char* normalizationParametersPath= NULL,
		     int n=0, bool forTraining=false);
  svm_node* getFeatureVector(CvSeq* cc);  
};
